...
import ru.ssau.tk.const1.labs.io.FunctionsIO;
...
public class SimpleOperationsWindow extends JDialog {
    ...
    private final AbstractTableModel firstTableModel = new PartiallyEditableTable(xValues, yValues);
    private final JTable firstTable = new JTable(firstTableModel);

    private final AbstractTableModel secondTableModel = new PartiallyEditableTable(secondXValues, secondYValues);
    private final JTable secondTable = new JTable(secondTableModel);

    private final AbstractTableModel resultTableModel = new NotEditableTable(resultXValues, resultYValues);
    ...
    private void getPopupMenu(JButton button, int flag) {
        ...
        fromFunction.addActionListener(ee -> {
            FunctionWindow window = new FunctionWindow(factory);
            setTable(window.getFunction(), flag);
        });
        ...
        resultButton.addActionListener(e -> {
            try {
                ...
                switch ((String) comboBox.getSelectedItem()) {
                    case "+":
                        resultFunction = operation.addition(firstFunction, secondFunction);
                        break;
                    case "-":
                ...
                ExceptionHandling.Processing("Введите обе функции");
            }
        });
    ...
    private void readFunction(int flag) {
        ...
        if (file != null) {
            try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                ...
                ExceptionHandling.Processing("Некорректные данные!");
    ...
    private void writeFunction(TabulatedFunction function) {
        ...
        if (file != null) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
                ...
                ExceptionHandling.Processing(e.getMessage());
            }
        }
    }
